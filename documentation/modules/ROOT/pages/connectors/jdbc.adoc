...
* `i.d.c.j.n.DefaultColumnNamingStrategy` (default) +
* `i.d.c.j.n.CustomColumnNamingStrategy` +
+
...
|_empty string_
|Specifies a prefix to prepend to column names when using `CustomColumnNamingStrategy`. Optional.

|[[jdbc-property-column-naming-suffix]]<<jdbc-property-column-naming-suffix, `+column.naming.suffix+`>>
|_empty string_
|Specifies a suffix to append to column names when using `CustomColumnNamingStrategy`. Optional.

|[[jdbc-property-column-naming-style]]<<jdbc-property-column-naming-style, `+column.naming.style+`>>
|`default`
|Specifies the naming style to apply to column names when using `CustomColumnNamingStrategy`. Supported values are: +

* `snake_case` +
* `camel_case` +
* `upper_cae` +
* `lower_case` +
* `default` (no transformation). Optional.
...
* `i.d.c.j.n.DefaultCollectionNamingStrategy` (default) +
* `i.d.c.j.n.CustomCollectionNamingStrategy` +

The default behavior is to replace the `+${topic}+` placeholder in the `collection.name.format` configuration property with the event's topic and sanitize the table name by replacing dots (`.`) with underscores (`_`).

|[[jdbc-property-collection-naming-prefix]]<<jdbc-property-collection-naming-prefix, `+collection.naming.prefix+`>>
|_empty string_
|Specifies a prefix to prepend to table names when using `CustomCollectionNamingStrategy`. Optional.

|[[jdbc-property-collection-naming-suffix]]<<jdbc-property-collection-naming-suffix, `+collection.naming.suffix+`>>
|_empty string_
|Specifies a suffix to append to table names when using `CustomCollectionNamingStrategy`. Optional.

|[[jdbc-property-collection-naming-style]]<<jdbc-property-collection-naming-style, `+collection.naming.style+`>>
|`default`
|Specifies the naming style to apply to table names when using `CustomCollectionNamingStrategy`. Supported values are: +
* `snake_case` +
* `camel_case` +
* `upper_case` +
* `lower_case` +
